...
export type NavKey = "home" | "plant" | "search" | "alerts" | "you";
...
export const NAV_ITEMS: {
  ...
  // "plant" is an ACTION, not a link — it opens the create sheet (see
  // PlantButton). Discovery that used to live in Explore now lives on Home.
  { href: "/", label: "Home", key: "home", emoji: "🏡" },
  { href: "", label: "Plant", key: "plant", emoji: "🌱" },
...
export function NavIcon({ name, size = 22 }: { name: NavKey; size?: number }) {
  ...
  const paths: Record<NavKey, ReactNode> = {
    ...
    plant: (
      <>
        <path d="M12 21v-9" />
        <path d="M12 12C12 8.5 9.5 6.5 5.5 6.5c0 3.5 2.5 5.5 6.5 5.5z" />
